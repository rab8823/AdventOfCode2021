...
public sealed class Day8 : Problem
{
    private const string Data = @"aedcg db ecbdgf badfegc abfcde edb cbgfe bfdg bdgec agfbce | ecagd gcbde dbcefa bgfd
...
dcfaeg dcfbag gfcdeba fagb dgacb fgc cedbf fcgdb dbacge gf | fg fbcgd dcgafe fadgbc";
    private const string SampleData = @"be cfbegad cbdgef fgaecd cgeb fdcge agebfd fecdb fabcd edb | fdgacbe cefdb cefbgd gcbe
...
gcafb gcf dcaebfg ecagb gf abcdeg gaef cafbge fdbac fegbdc | fgae cfgab fg bagce";
    ...
    private static readonly Dictionary<int, int> DigitToSegmentLength = new()
    {
        [0] = 6,
        [1] = 2,
        [2] = 5,
    ...
    {
        var result = 0;
        foreach ((var input, var output) in ReadData(Data))
        {
    ...
    private static IEnumerable<(string input, string output)> ReadData(string data)
    {
    ...
    private static (string input, string output) Parse(string data)
    {
    ...
    public override void SolvePart2()
    {
        ...
        foreach ((var input, var output) in ReadData(Data))
        {
            ...
            Dictionary<Segments, char> segmentToChar = new();
            string zero = null,
                one = null,
                two = null,
                three = null,
            ...
            Debug.Assert(one != null, "one should not be null");
            Debug.Assert(seven != null, "seven should not be null");
    ...
    private static char IdentifyTopSegment(string one, string seven)
    {
        Debug.Assert(one.Length == 2 && seven.Length == 3, "incorrect lengths");
        for (var i = 0; i < seven.Length; i++)
        {
            if (one.IndexOf(seven[i]) < 0)
            {
                return seven[i];
            }
        }
    ...
    private static readonly Dictionary<int, List<Segments>> DigitToSegments = new()
    {
        [0] = new() { Segments.Top, Segments.TopLeft, Segments.TopRight, Segments.BottomLeft, Segments.BottomRight, Segments.Bottom },
        [1] = new() { Segments.TopRight, Segments.BottomRight },
        [2] = new() { Segments.Top, Segments.TopRight, Segments.Middle, Segments.BottomLeft, Segments.Bottom },
        [3] = new() { Segments.Top, Segments.TopRight, Segments.Middle, Segments.BottomRight, Segments.Bottom },
        [4] = new() { Segments.TopLeft, Segments.TopRight, Segments.Middle, Segments.BottomRight },
        [5] = new() { Segments.Top, Segments.TopLeft, Segments.Middle, Segments.BottomRight, Segments.Bottom },
        [6] = new() { Segments.Top, Segments.TopLeft, Segments.Middle, Segments.BottomRight, Segments.Bottom, Segments.BottomLeft },
        [7] = new() { Segments.Top, Segments.TopRight, Segments.BottomRight },
        [8] = new() { Segments.Top, Segments.TopLeft, Segments.TopRight, Segments.Middle, Segments.BottomLeft, Segments.BottomRight, Segments.Bottom },
        [9] = new() { Segments.Top, Segments.TopLeft, Segments.TopRight, Segments.Middle, Segments.BottomRight, Segments.Bottom },
    };

    private static readonly Dictionary<Segments, List<int>> SegmentToDigits = new()
    {
        [Segments.Top] = new() { 0, 2, 3, 5, 6, 7, 8, 9 },
        [Segments.TopLeft] = new() { 0, 4, 5, 6, 8, 9 },
        [Segments.TopRight] = new() { 0, 1, 2, 3, 4, 7, 8, 9 },
        [Segments.Middle] = new() { 2, 3, 4, 5, 6, 8, 9 },
        [Segments.BottomLeft] = new() { 0, 2, 6, 8 },
        [Segments.BottomRight] = new() { 0, 1, 3, 4, 5, 6, 7, 9 },
        [Segments.Bottom] = new() { 0, 2, 3, 5, 6, 8, 9 }
    };
}